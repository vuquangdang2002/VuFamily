  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const res = await fetch(
        `${getApiBase()}/chats/${activeRoomId}/messages`,
        {
          method: "POST",
          headers: {
      ...
      if (json.success) {
        ...
        addToast(
          json.error || t("chat.send_fail") || "Không thể gửi tin nhắn.",
          "error",
        );
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full bg-transparent w-full relative">
      ...
      <div className="px-4 py-3 flex items-center gap-3 border-b border-black/5 dark:border-white/5 bg-white/60 dark:bg-black/20 backdrop-blur-xl shrink-0 z-10 shadow-sm">
        <button
          className="md:hidden w-10 h-10 flex items-center justify-center rounded-xl bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 text-zinc-600 dark:text-zinc-300 transition-colors"
          onClick={() => setActiveRoomId(null)}
        >
          <ChevronLeft size={24} />
        </button>
        <div
          className={`relative w-11 h-11 shrink-0 rounded-[1rem] shadow-sm overflow-hidden flex items-center justify-center text-white font-black text-sm ${activeRoom?.type === "group" ? "bg-gradient-to-br from-emerald-400 to-teal-600" : "bg-gradient-to-br from-amber-400 to-orange-500"}`}
        >
          {activeRoom?.avatar ? (
            <img
            ...
            activeRoom?.display_name?.substring(0, 2).toUpperCase()
          )}
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <div className="flex items-center gap-2">
            <h3 className="m-0 font-extrabold text-[15px] text-zinc-900 dark:text-white truncate">
              {activeRoom?.display_name || t("chat.group_label")}
            </h3>
            {activeRoom?.type === "group" && (
              <button
                className="w-6 h-6 flex items-center justify-center rounded-md bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/20 text-zinc-500 transition-colors shrink-0"
                title={t("chat.rename_group")}
          ...
          {activeRoom?.type === "direct" && (
            <span
              className={`text-[12px] font-medium truncate ${activeRoom.is_online ? "text-emerald-500" : "text-zinc-500"}`}
            >
              {activeRoom.is_online
                ? t("chat.online_status")
                : t("chat.offline_status")}
            </span>
          )}
        </div>
      ...
      {/* Messages Container */}
      <div
        className="flex-1 overflow-y-auto p-4 md:p-6 block space-y-4"
        id="message-list"
      >
        {messages.length === 0 && (
        ...
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`flex items-end gap-2 max-w-[85%] md:max-w-[70%] ${isMe ? "flex-row-reverse" : "flex-row"}`}
              >
                {!isMe && (
                ...
                <div
                  className={`px-4 py-2.5 text-[14px] leading-relaxed break-words shadow-sm ${
                    isMe
              ...
              <span className="text-[10px] font-bold text-zinc-400 mt-1 mx-2">
                {parseDateSafe(msg.created_at).toLocaleTimeString("vi-VN", {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </span>
      ...
      <div className="p-4 border-t border-black/5 dark:border-white/5 bg-white/60 dark:bg-black/40 backdrop-blur-xl shrink-0 z-10">
        <form onSubmit={handleSendMessage} className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-12 h-12 rounded-full bg-blue-600 text-white hover:bg-blue-700 shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
          >
